...
<html>
    ...
    <body>
            ...
            <ul class="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500">
                ...
                <li class="me-2">
                    <a href="../email_template/index.php?<?php echo http_build_query($requestparams); ?>" class="inline-flex items-center justify-center p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 group">
                        <svg class="w-4 h-4 me-2 text-gray-400 group-hover:text-gray-500" viewBox="0 0 24 24" fill="currentColor"><defs><style>.cls-1{opacity:0;}.cls-2{}</style></defs><title>email_template</title><g id="Layer_2" data-name="Layer 2"><g id="email_template"><g id="email_template-2" data-name="email_template"><rect class="cls-1" width="24" height="24"/><path class="cls-2" d="M19,4H5A3,3,0,0,0,2,7V17a3,3,0,0,0,3,3H19a3,3,0,0,0,3-3V7A3,3,0,0,0,19,4Zm-.67,2L12,10.75,5.67,6ZM19,18H5a1,1,0,0,1-1-1V7.25l7.4,5.55a1,1,0,0,0,.6.2,1,1,0,0,0,.6-.2L20,7.25V17A1,1,0,0,1,19,18Z"/></g></g></g></svg>Email Template
                    </a>
                </li>
                <li class="me-2">
                ...
                <li class="me-2">
                    <a href="../contact_us/index.php?<?php echo http_build_query($requestparams); ?>" class="inline-flex items-center justify-center p-4 text-blue-600 border-b-2 border-blue-600 rounded-t-lg active group">
                        <svg class="w-4 h-4 me-2 text-blue-" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21 8V7l-3 2-3-2v1l3 2 3-2zm1-5H2C.9 3 0 3.9 0 5v14c0 1.1.9 2 2 2h20c1.1 0 1.99-.9 1.99-2L24 5c0-1.1-.9-2-2-2zM8 6c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm6 12H2v-1c0-2 4-3.1 6-3.1s6 1.1 6 3.1v1zm8-6h-8V6h8v6z"/></svg>Contact us
                    </a>
                </li>
            </ul>
        </div>

        <div class="grid grid-cols-1 container bg-white py-5 px-8 rounded-md border border-gray-100 shadow my-5">
          <?php if(isset($err)){ ?><span class="font-medium text-red-500 text-xs">**<?php echo $err; ?></span><?php } ?>
          
          <div class="mt-2">
              <label for="name" class="block mb-1 text-gray-900">Your name:</label>
              <input type="text" class="bg-gray-50 duration-200 border border-gray-300 text-gray-900 rounded-sm focus:ring-black focus:border-black focus:drop-shadow-md focus:shadow-black/50 block w-full px-2 py-1.5" id="name" placeholder="Enter your name" name="name" required >
          </div>
          
          <div class="mt-2">
              <label for="email" class="block mb-1 text-gray-900">Email:</label>
              <input type="text" class="bg-gray-50 duration-200 border border-gray-300 text-gray-900 rounded-sm focus:ring-black focus:border-black focus:drop-shadow-md focus:shadow-black/50 block w-full px-2 py-1.5" id="email" placeholder="Enter email address" name="email" required >
          </div>
          
          <div class="mt-2">
              <label for="message" class="block mb-1 text-gray-900">Issue / Feedback / Suggestion for next update:</label>

              <textarea id="message" class="bg-gray-50 duration-200 border border-gray-300 text-gray-900 rounded-sm focus:ring-black focus:border-black focus:drop-shadow-md focus:shadow-black/50 block w-full px-2 py-1.5" name="message" rows="4" cols="50" placeholder="describe here" required ></textarea>

              <p class="text-right text-xs text-gray-400 font-medium">*maximum 500 chars</p>
          </div>

          <div class="mt-5">
            <button type="submit" name="submit" class="bg-gray-800 text-sm font-medium px-5 py-2 text-white rounded shadow-lg hover:bg-gray-900 hover:shadow-none duration-150" onclick="submitContactUsForm();">
              Submit
            </button>
          </div>
        </div>
      ...
      <?php
        $submit_params = array("user"=> $user[0]['id']);
        $submit_hmac = hash_hmac('sha256', http_build_query($submit_params), $secret);
      ?>
      ...
      <script>
        function submitContactUsForm(){
          ...
          $.ajax({
            type: 'post',
            url: 'ajax/submit_contact_us_form.php',
            data: {
              name : name,
              email : email,
              message : message,
              user : <?php echo $user[0]['id']; ?>,
              hmac : '<?php echo $submit_hmac; ?>'
            },
            success: function(data){
              ...
              if (obj.status == 1){
                ...
                window.location.href = "../contact_us/index.php?<?php echo http_build_query($requestparams); ?>";
              }else{
                var toastOptions = {
                  message: obj.msg,
                  duration: 5000,
